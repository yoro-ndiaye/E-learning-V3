      ...
      <div class="p-6 text-gray-900 dark:text-gray-100">
        <div class="container mx-auto">
          ...
          <form action="{{ route('stagiaires.updateProfil') }}" method="POST" class="max-w-md mx-auto p-4 bg-white rounded shadow-md min-w-full" enctype="multipart/form-data">
            ...
            <!-- Champ d'entrée pour le numéro de téléphone Ajouter par yoro -->
            <div class="m-4">
              <label for="telephone" class="block text-gray-700 text-sm font-bold mb-2">Téléphone:</label>
              <input type="tel" id="telephone" name="telephone" placeholder="Entrez le numéro de téléphone" class="w-full px-3 py-2 border rounded-md border-gray-300 focus:outline-none focus:ring focus:border-gray-700 text-gray-900" value="{{ $stagiaire->telephone }}">
              @error('telephone')
              <p class="text-red-500 text-xs italic">{{ $message }}</p>
              @enderror
            </div>
